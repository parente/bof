...
@api.route('/api/flocks', methods=['POST'])
def create_flock():
...
@api.route('/api/flocks/<fid>', methods=['PUT', 'POST'])
...
@api.route('/api/flocks/<fid>', methods=['DELETE'])
...
@api.route('/api/flocks/<fid>/birds', methods=['POST'])
def join_flock(fid):
    # authenticated user is the leader of the flock
    try:
        username = session['username']
...
@api.route('/api/flocks/<fid>/birds/self', methods=['DELETE'])
def leave_flock(fid):
    # authenticated user is the leader of the flock
    try:
        username = session['username']
